...
const UserProfil = () => {
  ...
  useEffect(() => {
    const fetchUserAndCritics = async () => {
      try {
        ...
        const userResponse = await fetch(`http://localhost:3333/auth/user/${userId}`);
        if (!userResponse.ok) {
        ...
        if (userData.critics && userData.critics.length > 0) {
          const criticResponses = await Promise.all(
            userData.critics.map((criticId) =>
              fetch(`http://localhost:3333/critics/${criticId}`).then((res) => {
                if (!res.ok) {
  ...
  const handleSelectImage = async (imageSrc) => {
    ...
    try {
      const response = await fetch(`http://localhost:3333/auth/${userId}/updateProfile`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <>
      ...
      <div className="containeur-user-critic">
        <div>
          <Link to="/">
            <img className="arrow-back" src="../src/assets/arrow.png" alt="Retour" />
          </Link>
        </div>
